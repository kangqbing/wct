...
/**
 *  IQ is a basic library which provider following functions: {
	...
	*/
	...
	var propertyUtils = {
		...
		getPropertyAsFunction:function(obj, fName){
			var fn = IQ.getProperty(obj, fName);
			return IQ.isFn(fn)?fn : IQ.emptyFn;
		},
		clone :cloneFn,
	...
	var namespaceUtils = {
		...
		setNS : function(nsname, obj){
			...
			if (names[0]=="window")
				names.shift();
			if (names.length==0 || IQ.isEmpty(names[0]))
				return;
	...
	var _bindHandlers = function(el, handlers, isBind, isOnce){
		...
		IQ.iterate(["click", "dblclick", "focus", "blur", "keyup", "keydown", "mouseover", "mouseout", "resize", "scroll" ,"mousedown", "mousemove", "mouseup","touchstart", "touchend", "touchmove"], function(eventName){
			if (eventName in handlers){								
				...
				IQ[isBind?"attachEvent":"detachEvent"](el, eventName, _handlerWrapper(handlers[eventName]));
			}			
		});
	};
	var browserUtils = {
...
$X = IQ.get;
/**
 * $XA is an shortcut to IQ.cvtToArray.	
...
$XE = IQ.err;
/**
 * $XP is an shortcut for IQ.getProperty. For example:
...
$XP = IQ.getProperty;
/**
 * $XF is an shortcut for IQ.getPropertyAsFunction. For example:
 * 		var closeFn = $XF(config, "close")
 * 		it means assign config.close to closeFn, if config has no property named as "close", assign IQ.emptyFn to closeFn. 
 *  
 */
$XF = IQ.getPropertyAsFunction;
...
IQ.Test =(function(){
	var detectAttrsFn = function(obj, checkFn, injectorFn) { // injectorFn: function(result, name, value)
		var count = 0;
		return  IQ.each(obj, "", function(acc, item, nc){
			try{
				if (checkFn(item,nc)){
					count += 1;
...
/**
 *  IQ.Array is a supplement for Array.prototype. It includes: {
...
IQ.Array = (function(){
	var getEqualFn = function(equalFn){
		return IQ.isFn(equalFn)?equalFn:function(a, b) {
			return (IQ.isObject(a) &&("equal" in a) &&  IQ.isFn(a.equal))?a.equal(b):(a==b);
		};
	};
	...
	_self = {
		...
		toSet : function(arr, equalFn) {
			...
			return IQ.loop(arr, [], function(acc, item){
		...
		sort : function(arr, cmpFn){
			var arr1 = IQ.map(arr, function(item){return item;});
		...
		compact: function(arr, validFn) {
			var fn = IQ.isFn(validFn)?validFn:function(item){return item;};
			return IQ.loop(arr, [], function(acc, item) {
				if (fn(item))
					acc.push(item);
				return acc;
		...
		flat : function(arr) {
			return IQ.isArray(arr)?IQ.loop(arr, [], function(acc, item){
...
IQ.I1ToNManager = function(eqFn) {
	var _eqFn = IQ.isFn(eqFn)?eqFn : function(item, value){return item==value;};
	
	var _mgr = new IQ.IManager();
	...
	var indexOfFn = function(arr, value) {
		return IQ.Array.indexOf(arr, function(item){return _eqFn(item, value);});
	};
	
	return IQ.inherit(_mgr, {
		hasValue :hasEntryFn,
		put : function(k, v) {
			if (!hasEntryFn(k)) {
				_mgr.register(k, [v]);
		...
		remove : function(k, v){
			...
			if (idx >= 0)
				_mgr.register(k, IQ.Array.splice(list, idx));	
		}	
	});
};
IQ.IListManager = function() {
	var _super = new IQ.IManager();
	var _list = new IQ.IList();
	...
	var listFn = function(keys) {
		return IQ.map(keys, function(item) {return _super.get(item);});
	};
	return IQ.inherit(_super, {
		...
		iterate: function(fn){IQ.iterate(_list.getList(), function(item){fn(_super.get(item));}); },
		getFirst : function() {
 ...
 */
IQ.DataStore = function(data){
	var _items = $XP(data, "items", []);
	if (_items.length>0 && $XP(data, "type", "json")!="json"){
		var _fields = $XP(data, "fields", []);
		_items =  IQ.map(_items, function(row){
			return IQ.loop(_fields, {}, function(acc, col, idx){
				acc[col] = IQ.isArray(row)?row[idx]:row[col];
				return acc;
			});
		});
	}
	
	return IQ.map(_items, function(item){
		var id = $XP(item, "id");
		if (IQ.isEmpty(id))
			item.id = IQ.id();
		return item;
...
IQ.Date = (function(){
	...
	var _formatStr = function(str, sp) {
		if (IQ.isEmpty(str))
			return "";
		str = str.split(sp, 3);
		return IQ.map(sp==ds?[4,2,2]:[2,2,2], function(item, idx){
	...
	var _format = function(date, fields, sp){
		...
		var str = IQ.map(fields, function(item){
	...
	var isValid = function(str, type){
		...
		if (sps.length!=3 || IQ.loop(sps, false, isDate?checkData:checkTime))
			return false;
		if (isDate){
			var m = sps[1]*1;
			var d = sps[2]*1;
	...
	return {
		...
		*/
        ...
        getTimeTickInSec : function (_dateStr) {
        	var _str = IQ.Date.getDateByFormat(_dateStr, 'yyyy/MM/dd HH:mm:ss');
        	return parseInt((new Date(_str)).getTime() / 1000);
        }
	};
})();
/**
 *	IQ.Net is a library for networking. It includes: {
...
IQ.Net = (function(){
		...
		request.onreadystatechange  = function(){
			if (request.readyState == 4){
				if (request.status == 200){
					if (IQ.isFn(cbFun)){cbFun(request.responseText);}
				} else { 
	...
	var loadJsFilesInSeqFn = function(jsFiles, nextFn){
		var _nextFn = IQ.isFn(nextFn)?nextFn:IQ.emptyFn;
	...
	return {
		...
		tryFn:function(fnName, argList,dependencyConfig){
			var fn = function(){				
				IQ.execute(fnName, argList);
				IQ.tryFn(dependencyConfig.afterFn);
			};
			if (!IQ.nsExisted(fnName)){
				if (!dependencyConfig){
					return;
				}
				var config = dependencyConfig;
				IQ.tryFn(config.beforeFn);
				IQ.iterate(config.cssFiles, loadCssFn);
...
IQ.win = (function(){
	var fnHT = {}, eventFnHT = {}, handlersHT = IQ.I1ToNManager();
	var keySn = 0;
	...
	var executeEvent = function(eName, e){
		IQ.iterate(handlersHT.get(eName), function(fKey){
			//try{
			if (IQ.isFn(fnHT[fKey]))
				fnHT[fKey](e);
			//}catch(ex){
			//	console.log(ex);
			//}
	...
	var _bindHandlers = function(handlers, isBind){
		...
		var ids = IQ.loop(["click", "resize", "scroll" ,"mousedown", "mouseover", "mouseout"], {}, function(acc, eventName){
			if (eventName in handlers)
...
function defaulRspFn(data, cbFn){IQ.isFn(cbFn) && cbFn(data);}
function getFunProp(_cfg, _name, defFn){
	var _fn = $XP(_cfg, _name);
	return IQ.isFn(_fn)?_fn :defFn;
}
...
function createRouteHT(routes, ifAjax){
	return IQ.loop(routes, {}, function(acc, routeDef){
...
function AjaxCaller(routes, _ajaxFn, urlFac, chkRes){
	...
	var c = function(_name, params, cbFn, failFn){
		...
		var _cbFn = IQ.isFn(cbFn) ? cbFn : IQ.emptyFn;
		...
		_ajaxFn({
			...
			success : function(data) {
				...
				// if (IQ.isFn(chkRes) && !chkRes(data)) return ;
				IQ.isFn(chkRes) && chkRes(data);
...
function UrlFactory(){
	...
	var genUrl = function(_route, params){
		if (!_route)
			return "";
		var url = _route.url;
		var _url = IQ.isFn(url)?url(params):url.replaceByParams(params);
		var _urlBase = (_route.urlType in _urls)?_urls[_route.urlType] : _urls.baseUrl;
		return _urlBase + _url;
	};
	return {
		init : function(cfg){_urls = IQ.inherit(_urls, cfg);},
		genUrl : genUrl
	};
}
	...
	return IQ.inherit({
		...
		createCaller: function(routes, chkRes){
			return new AjaxCaller(routes, function(ajaxParam){
				if (IQ.isFn(_ajaxFn)) _ajaxFn(ajaxParam);
			}, _urlFac, chkRes);
		}
	} : {});
}
 
IQ.urlEngine = new UrlEngine();
IQ.ajaxEngine = new UrlEngine(true);
})();
/**
 * 	IQ.Xml is a library to deal with XML string or document. It includes: {
 ...
 * 	}
 */
IQ.Xml = (function(){return {
	parser:function(str){
		str = IQ.isString(str)?str:"";
		var doc = null;
	...
	getXmlString:function(xmlDoc){
		...
		if(IQ.nsExisted("document.implementation.createDocument")) {
...
/**
 * 	IQ.Dom is a library to deal with DOM. It includes :{
...
IQ.Dom = (function(){
	...
	var getFn = function(node, tagN, type){
		return IQ.isString(tagN)?loopFn(node, type, function(cnode){
	...
	var textFn = IQ.isMSIE?function(node){return node? node.innerText:"";}:function(node){return node?node.textContent:"";};
	...
	var inTagFn = function(tag, content, attrs){//attrs should like [[pramName, paramValue],...
		var _attrs = IQ.loop(attrs, [],  function(acc, item){
...
*/
IQ.HtmlDocument = (function(){
	var hasFn = function(el, clzName){
		return el!=null && ("className" in el)&& IQ.Array.isFound(clzName, (el.className+"").split(" "));
	};
	var removeFn = function(el, clzName){
		if (!el) return;
		var clz = IQ.Array.remove(el.className.split(" "), clzName);
		el.className = clz.join(" ");
	};
	var addFn = function(el, clzName) {
		if (!el) return;
		var clzs = IQ.Array.toSet(el.className.split(" ").concat(clzName));
	...
	var getStyle = function(_elem,styles){
        var _value=null, elem= IQ.get(_elem);
        ...
        if(styles == "opacity"){
	        ...
	        if(_value=="auto" && IQ.Array.indexOf(["width","height"], function(_i){return styles == _i;}) > -1 && elem.style.display!="none"){
	...
	return {
		...
		rect : function(el, ri){
			IQ.iterate(["left", "top", "width", "height"], function(attr, idx){
				if (ri[idx]!=null)
...
IQ.Task = function(taskFn, interval, times){
	var ts = -1;
	var h = null;
	var execFlag = false;
	var _times = 0, _total = isNaN(times)?-1:times;
	var getTimeInMS = IQ.getTimeInMS;
	function _fn(){
		if (!execFlag)
			return;
		taskFn(_times);
...
var parseTpl = function(tplstr){
	...
	var reformTpl = function(name){
		...
		curTpl.tpl = html; 
		curTpl.list =  IQ.Array.toSet(html.match(rpRegex)).sort();
	};
	...
	var nameArr = IQ.loop(tplArr, ["root"], function(acc, item, idx) {
		if (item=="</tpl>")
...
IQ.ITemplate = function(config){
	...
	var getProps = function(data, name){
		if (!IQ.hasProperty(data,name))
			return null;
		var v = $XP(data, name);
		return IQ.isEmpty(v)?"":v;
	};
	var renderFn = function(tplId, data){
		...
		var html = tpl.tpl;
		var arr = IQ.loop(tpl.list, [], function(acc, item){
			var t = item.charAt(0);
			...
			if (t=='{') {
				...
				var h = IQ.map($XP(data, _name, []), function(itm, idx) {
					var idata = IQ.inherit(itm, {idx: idx});
...
IQ.extend(String.prototype, {
	...
	loopReplace:function(varr){return IQ.loop(varr, this, function(acc, item){
	...
	isEmail : function(){
		var email = this.trim();
		return IQ.isEmpty(email) || EmailPattern.exec(email);
	},
	...
	multi:function(len){ return IQ.Array.init(len, this).join("");},
	...
	replaceByParams : function(data) {
		var items = IQ.Array.compact(this.match(ReplaceKeyPattern));
		return IQ.loop(items, this, function(acc, item){
			var _key = item.slice(1,-1);
			return IQ.isEmpty(_key)?acc:acc.replaceAll(item, $XP(data, _key, ""));
		});
	},
	inPureTag :function(tagN, attrs){return IQ.Dom.inPureTag(tagN, this, attrs);},
	inTag :function(tagN, attrs){return IQ.Dom.inTag(tagN, this, attrs);},
...
/**
 * 	IQ.UUID is a generator to create UUID.
 */
IQ.UUID = (function(){
...
IQ.Util.Date = function(timeInSecond) {
	var date = new Date(timeInSecond*1000);	
	var getFieldValues = function(_time){
		var getPrefix = "get" + (IQ.Date.isUTC()?"UTC":"");
		return IQ.map(["FullYear", "Month", "Date", "Hours", "Minutes", "Day"], function(f){
	...
	return {
		toText: function(){return IQ.Date.format(date);},
		...
		toShort : function(){
			var ds = IQ.Date.getDS(), ts = IQ.Date.getTS();
			var curTime = getFieldValues(new Date());
			var strs = new Array();
		...
		toSimpleInterval : function(){
			return IQ.Date.getDateText(timeInSecond,IQ.getTimeInMS() /1000);
		}
	};
}; 
...
IQ.Util.Event = {
	...
	stop: function(e){
		IQ.Util.Event.preventDefault(e);
		IQ.Util.Event.stopPropagation(e);
	}
};
///////////////////////////////////////////////////// UTILS finished///////////
/**
 ...
 * @param {} config {
 * 		id: the identified object(String, number, ...). If not provider, use IQ.UUID to create one.
 ...
 */
/** //Unused temporary 
IQ.IObject = function(config){
	var _id = $XP(config, "id", IQ.UUID.generate());
...
IQ.SELF_PATH = path;
IQ.SCRIPT_ROOT = path.substring(0, path.indexOf("utils/ixutils.js")); 
})();
