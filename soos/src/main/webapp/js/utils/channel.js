(function () {
	...
	var sendMsg = function (channelNames, msgStr, onOk, onFail) {
		IQ.iterate(channelNames, function (channelName) {
	...
	var initPipe = function (sockUrl) {
		if (!host && !port) {
			host = KO.G.HOST;
			protocol = KO.G.HOME.split("//")[0];
			port = KO.G.PORT || ((protocol == "https:") ? 443 : 80);
		}
		urlPrefix = [protocol, "//", host, (port!=80 && port!=443) ?( ":" + port):"", sockUrl || "/pub","?id="].join("");
		return new PipeClz({
			host : host,
			port : port,
			modes : ifWebSecketOnly || KO.isClient ? "websocket" : "websocket|eventsource|stream",
	 ...
	 */
	KO.Socket.init = function (mode, url) {
		if (mode == "mobile") {
			PipeClz = null;
		...
		sock = new Socket(url);
		KO.Socket.listen = sock.openChannel;
		KO.Socket.unlisten = sock.closeChannel;
		KO.Socket.close = sock.closeAllChannels;
		KO.Socket.getChannel = sock.getChannel;
		KO.Socket.sendOnChnnel = sock.send;
		KO.Socket.getAllChannelKeys = sock.getAllChannelKeys;
