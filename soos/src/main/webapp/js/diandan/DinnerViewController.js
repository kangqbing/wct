define(['app', 'diandan/OrderHeaderSetController'], function (app) {
	...
	[
		...
		function ($scope, $rootScope, $modal, $location, $filter, $timeout, storage, CommonCallServer, OrderService, FoodMenuService, OrderChannel, OrderNoteService, AppAlert, AppAuthEMP, AppProgressbar, AppConfirm, EMPPermission) {
			...
			FoodMenuService.initFoodMenuData(function (data) {
				...
				IQ.Debug.info("All FoodCategories Data:");
				IQ.Debug.info($scope.FoodCategories);
			...
			OrderNoteService.getOrderNotesLst({}, function (data) {
				IQ.Debug.info("Order Notes: ");
				IQ.Debug.info(OrderNoteService.OrderNoteDict);
			}, function (data) {
				// HC.TopTip.addTopTips($rootScope, data);
				AppAlert.add('danger', _.result(data, 'msg', ''));
			});
			...
			$scope.searchFood = function () {
				IQ.Debug.info($scope.curSearchKey);
				...
				// 	IQ.Debug.info("Matched Foods:");
				// 	IQ.Debug.info(matchedFoods);
				// 	setTimeout(function () {
			...
			$scope.$on('foodCategory.change', function () {
				...
				IQ.Debug.info("Current Food Menu Datas:");
				IQ.Debug.info($scope.curFoodCategory);
				IQ.Debug.info($scope.curFoods);
			...
			$scope.submitOrder = function () {
				...
				var successCallBack = function (data) {
					...
					if (code == '000') {
						...
						$scope.resetOrderInfo();
						// 向子窗口推送新加菜品的消息
						KO.SecondScreen.publishPostMsg('OrderDetail', OrderService.getOrderPublishData());
						$timeout(function () {
	...
	app.controller('OrderItemSendController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Food Item Send setting:");
				IQ.Debug.info("foodSendNumber:" + $scope.foodSendNumber + ";sendReason:" + $scope.sendReason);
	...
	app.controller('OrderItemCancelController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert, AppAuthEMP) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Food Item Cancel Setting:");
				IQ.Debug.info("foodCancelNumber:" + $scope.foodCancelNumber + ";cancelReason:" + $scope.cancelReason);
	...
	app.controller('OrderItemModifyCountController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert, AppAuthEMP) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Food Modify Count Setting:");
				IQ.Debug.info("foodCount:" + $scope.foodCount);
	...
	app.controller('OrderItemModifyPriceController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert, AppAuthEMP) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Food Modify Price Setting:");
				IQ.Debug.info("foodPrice:" + $scope.foodPrice + "; priceNote:" + $scope.priceNote);
	...
	app.controller('OrderItemModifyMethodController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Food Method Setting:");
				IQ.Debug.info(foodMethodSetting);
	...
	app.controller('OrderItemModifyRemarkController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert) {
			...
			$scope.save = function () {
				// TODO submit Modify result
				IQ.Debug.info("Food Remark Setting:");
				IQ.Debug.info("foodRemark:" + $scope.foodRemark);
	...
	app.controller('OrderRemarkController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, AppAlert) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Order Remark Setting:");
				IQ.Debug.info("orderRemark:" + $scope.orderRemark);
	...
	app.controller('SetFoodCfgController', [
		...
		function ($scope, $modalInstance, $filter, _scope, OrderNoteService, OrderService, FoodMenuService, AppAlert) {
			...
			$scope.save = function () {
				...
				IQ.Debug.info("Current SetFood Settings is:");
				IQ.Debug.info(curSetFoodData);
				IQ.Debug.info(setFoodData);
				// 向订单插入套餐配置信息
	...
	app.controller('SetFoodSettingController', [
		...
		function ($scope, $modalInstance, $filter, $timeout, _scope, $sce, OrderNoteService, OrderService, FoodMenuService, SetFoodService, AppAlert) {
			IQ.ns("KO");
			var curUnitKey = _scope.curSetFoodUnitKey;
			var setFoodData = IQ.clone(FoodMenuService.getFoodByUnitKey(curUnitKey));
	...
	app.controller('TempFoodSettingController', [
		...
		function ($scope, $modalInstance, $filter, $timeout, _scope, $sce, OrderNoteService, OrderService, FoodMenuService, AppAlert) {
			...
			var tempFoodData = IQ.clone(FoodMenuService.getFoodByUnitKey(curTempFoodUnitKey));
	...
	app.controller('PayOrderController', [
		...
		function ($scope, $rootScope, $modalInstance, $filter, $location, $timeout, _scope, storage, OrderService, OrderPayService, PaySubjectService, OrderDiscountRuleService, VIPCardService, AppAlert, AppAuthEMP, AppProgressbar, EMPPermission) {
			...
			IQ.Debug.info("OrderPayDetail:")
			IQ.Debug.info($scope.orderPayDetail);
			...
			// 当前选中支付科目组名称
			$scope.curPaySubjectGrpName = _scope.act == "cashPayOrder" ?  "cashPay" : "kooPay";
			// 缓存会员卡数据
			...
			$scope.changeCurrentPaySubjectGrp = function (paySubjectGrp) {
				var name = _.result(paySubjectGrp, 'name');
				//删除副屏二维码
				KO.SecondScreen.publishPostMsg('PayQRCode', '');
			...
			$scope.submitOrderPay = function ($event, progressbar) {
				...
				var submitOrderSuccess = function (data) {
					...
					if (code == "000") {
						...
						// 3. 打印结账清单
						KO.DevCom.exeCmd('PrintCheckoutBill', JSON.stringify(_.result(data, 'data')));
						// 向子窗口推送消息
						KO.SecondScreen.publishPostMsg('OrderDetail', OrderService.getOrderPublishData());
						if (operationMode == 0) {
				...
				if (!_.isEmpty(cardNo) && _.isEmpty(cardTransID)) {
					...
					vipCardDeductMoneyCallServer && vipCardDeductMoneyCallServer.success(function (data) {
						...
						if (code == '000') {
							// 1. 发送打印会员卡交易凭证
							KO.DevCom.exeCmd('PrintCRMTransBill', _.result(_data, 'transReceiptsTxt', ''));
							// 2. 提交订单
			...
			$scope.printCheckoutPreBill = function ($event) {
				...
				callServer.success(function (data) {
					...
					if (code == '000') {
						...
						KO.DevCom.exeCmd("PrintCheckoutPreBill", JSON.stringify(msg));
						$btn.button('reset');
					} else {
						// HC.TopTIp.addTopTips($rootScope, data);
						AppAlert.add('danger', _.result(data, 'msg', ''));
			...
			return {
				...
				link : function (scope, el, attr) {
					...
					var singlePaySubjectSubmit = function (curName) {
						...
						// 向子窗口推送消息
						KO.SecondScreen.publishPostMsg('OrderDetail', _.extend({}, OrderService.getOrderPublishData(), OrderPayService.getOrderPayParams()));
					};
					// 多支付科目组的提交
					...
					var mapFormCfg = function () {
						...
						switch(curPayGrpName) {
							...
							case "kooPay":
								...
								if (curPayGrpName == 'kooPay') {
									scope.payType = '';
									var curPayDetail = OrderPayService.mapOrderPayDetail(),
										payGrp = _.result(curPayDetail, 'payGrps', []),
										onlyHLLPayManual = false,
										testArr = 'cashPay,kooPay,voucherPay,vipCardPay,bankCardPay,groupBuyPay,hangingPay'.split(',');
									IQ.Debug.info('当前支付科目明细:');
									IQ.Debug.info(payGrp);
									var hasPayGrps = _.reject(payGrp, function (el) {
										return _.result(el, 'amount') == 0;
									});
									IQ.Debug.info('当前已使用支付科目:');
									IQ.Debug.info(hasPayGrps);
					...
					scope.$on('pay.submit', function (d, targetPaySubjectGrp) {
						...
						switch(curName) {
							...
							case "voucherPay":
							case "kooPay":
								if (curName == 'kooPay' &&  (!!scope.payType || !!scope.zhusao)) {
									scanQRCodePaySubmit();
									return;
								} else {
									singlePaySubjectSubmit(curName);
