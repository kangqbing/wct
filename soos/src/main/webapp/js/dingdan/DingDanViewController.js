		...
		function($scope, $rootScope, $modal, $location, $filter, $timeout, storage, CommonCallServer, LocalOrderLstService, OrderService, AppAlert,OrderNoteService) {
			...
			var jumpToDinnerPage = function (saasOrderKey, tableName, hisFlag) {
				...
				var search = {
					...
					'qReportDate' : _.isDate($scope.qReportDate) ? IQ.Date.getDateByFormat($scope.qReportDate, 'yyyyMMdd') : '',
			...
			$scope.queryOrderLst = function (pager) {
				var params = {
					...
					reportDate : _.isDate($scope.qReportDate) ? IQ.Date.getDateByFormat($scope.qReportDate, 'yyyyMMdd') : '',
		...
		function ($scope, $rootScope, $modalInstance, $filter, $location, $modal, _scope, storage, OrderService, OrderChannel, AppAlert, AppAuthEMP, EMPPermission) {
			...
			$scope.mapOrderDetailValue = function (el) {
				...
				switch(type) {
					...
					case 'netOrderTypeCode':
						ret = KO.TypeDef.NetOrderTypeCode[v] || emptyStr;
						break;
					case 'discountRange':
						ret = v == 0 ? '部分打折' : '全单打折';
						break;
					...
					case 'moneyWipeZeroType':
						ret = _.result(_.find(KO.TypeDef.MoneyWipeZeroTypes, function (el) {return el.value == v;}), 'label', emptyStr);
						break;
					case 'userSex':
						ret = _.result(_.find(_.find(KO.TypeDef.GENDER, function (el) {return el.value == v;})), 'label', emptyStr);
						break;
					case 'cash':
			...
			var jumpToDinnerPage = function (saasOrderKey, tableName, hisFlag) {
				...
				var search = {
					...
					'qReportDate' : _.isDate(_scope.qReportDate) ? IQ.Date.getDateByFormat(_scope.qReportDate, 'yyyyMMdd') : '',
			...
			$scope.printOrderCheckoutBill = function () {
				...
				IQ.Debug.info("Order Detail Info:");
				IQ.Debug.info(msg);
				// KO.DevCom.exeCmd("PrintCheckoutBill", JSON.stringify(msg));
				KO.DevCom.exeCmd("PrintCheckoutBillHis", JSON.stringify(msg));
			};
			// 反结账
			...
			var openModal = function (modalCfg) {
				...
				// $scope.modalIsOpen(true);
				// $modal.open(modalCfg);
				KO.ModalCom.openModal($rootScope, $modal, modalCfg);
			};
			// 修改发票
