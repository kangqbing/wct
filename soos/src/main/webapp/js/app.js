define(['routes', 'services/dependencyResolverFor'], function(config,
		...
		$provide.value("$locale", {
					"DATETIME_FORMATS" : {
						"MONTH" : ["1月", "2月", "3月", "4月", "5月", "6月", "7月",
								"8月", "9月", "10月", "11月", "12月"],
						"SHORTMONTH" : ["1月", "2月", "3月", "4月", "5月", "6月",
								"7月", "8月", "9月", "10月", "11月", "12月"],
						"DAY" : ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五",
								"星期六"],
	...
	angular.module("ui.bootstrap.tpls",
			["template/accordion/accordion-group.html",
					...
					"js/template/modal.html"]);
	angular.module("js/template/modal.html", []).run(["$templateCache",
	...
	app.config([
			...
			function($routeProvider, $locationProvider, $controllerProvider,
				...
				if (config.defaultRoutePaths !== undefined) {
					$routeProvider.otherwise(config.defaultRoutePaths);
				}
	...
	app.controller('AppController', ['$scope', '$rootScope', '$location',
			'storage', function($scope, $rootScope, $location, storage) {
				IQ.ns("KO");
				...
				// 订单反结账状态开关
				$rootScope.$on('Order.FJZStatusToggle',
						function($event, isFJZ) {
							$scope.isFJZ = isFJZ;
						});
				...
				$rootScope.$on('$routeChangeSuccess', function(event) {
					var shopInfo = storage.get('SHOPINFO'), empInfo = storage.get('EMPINFO');
					...
					if (!_.isEmpty($scope.shopLogo)) {
						$scope.shopLogo = KO.G.AJAX_DOMAIN + '/'+ $scope.shopLogo;
					} else {
						$scope.shopLogo = './img/blank.png';
					}
					var urlParams = $location.search(), deviceCode = _.result(
							urlParams, 'deviceCode', null), deviceKey = _
							.result(urlParams, 'deviceKey', null), deviceName = _
							.result(urlParams, 'deviceName', null);
					// foodMakeManageShowMode -- 出品管理显示模式0：整单；1：单品
					// departmentKeyLst -- 当前站点管理的菜品部门Key列表,多个时用逗号隔开，如果为空，表示全部菜品
					var foodMakeManageShowMode = _.result(urlParams,
							'foodMakeManageShowMode', 0), departmentKeyLst = _
							.result(urlParams, 'departmentKeyLst', '');

					var screen2Exists = _.result(urlParams, 'Screen2Exists', 0), screen2Left = _
							.result(urlParams, 'Screen2Left', 0), screen2Top = _
							.result(urlParams, 'Screen2Top', 0), screen2Width = _
							.result(urlParams, 'Screen2Width', 0), screen2Height = _
							.result(urlParams, 'Screen2Height', 0);
					...
					if (!_.isEmpty(foodMakeManageShowMode)) {
						storage.set('foodMakeManageShowMode',
								foodMakeManageShowMode);
				...
				$scope.navBtnClick = function(evt) {
					var curTar = $(evt.currentTarget), p = curTar.attr('href')
							.slice('1');
					$navEl.find('a[href="#' + p + '"] .badge-notify')
							.removeClass('in');
				...
				/**
				 * 打开子屏幕页面 1. 生成子屏幕页面链接 2. 打开子窗口，加载子屏幕页面 3. 订阅子屏幕页面需要的消息
				 * 
				 * @return {[type]} [description]
				 */
				$scope.openSecondScreen = function() {
					var HSS = KO.SecondScreen, subWin = HSS.getSubWin();
					if (storage.get('screen2Exists') != 1
							|| !_.isEmpty(_.result(subWin, 'window', null)))
						return;
					subWin = KO.SecondScreen.open();
					// 订阅订单条目postMsg
					KO.SecondScreen.subcribePostMsg('OrderDetail');
					// 订阅订单付款二维码postMsg
					KO.SecondScreen.subcribePostMsg('PayQRCode');
					// 订阅广告postMsg
					KO.SecondScreen.subcribePostMsg('AD');
				};
				...
				$scope.initSocket = function() {
					if (KO.PushMsg.hasSocket())
						return;
					KO.PushMsg.initWebSocketConnect();
					// 订阅新订单消息
					KO.PushMsg.subcribeMsg('NewOrder', function(topic, args) {
						
						var msgData = _.result(args, 'msgData');
						var newOrderCount = _.result(msgData, 'newOrderCount');
						var $siteHeader = $('#site_header'), $navBtn = $siteHeader.find('a[name=jiedan] .badge-notify');
					...
					// 订阅新消息
					KO.PushMsg.subcribeMsg('NewMsg', function(topic, args) {

							});
					// 订阅自助支付消息
					KO.PushMsg.subcribeMsg('SelfPay', function(topic, args) {
								var msgData = _.result(args, 'msgData');
								$rootScope.$broadcast('Pay.selfPay', msgData);
							});
					// 订阅基本信息更新消息
					KO.PushMsg.subcribeMsg('BaseUpdate', function(topic, args) {

							});
					// 菜品制作完成消息更新
					KO.PushMsg.subcribeMsg('FoodMaked', function(topic, args) {
								var msgData = _.result(args, 'msgData');
								$rootScope.$broadcast('Produce.updateOrder',msgData);
							});
					// 打印设备消息推送
					KO.PushMsg.subcribeMsg('PrinterMsg', function(topic, args) {
								var msgData = _.result(args, 'msgData');
