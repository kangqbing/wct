define(['app', 'uuid'], function (app, uuid) {
	...
	app.service('OrderService', 
		['$rootScope', '$location', '$filter', 'storage', 'CommonCallServer', 'OrderChannel', function ($rootScope, $location, $filter, storage, CommonCallServer, OrderChannel) {
			...
			this.getOrderByOrderKey = function (params, success, error) {
				...
				self.OrderFoodHT = new IQ.IListManager();
					...
					_.each(OrderHeaderKeys, function (k) {
						...
						switch (k) {
							...
							case "createBy":
								...
								break;
							case "startTime":
								v = KO.Date(parseInt((new Date()).getTime() / 1000)).toText();
								v = IQ.Date.getDateByFormat(v, 'yyyyMMddHHmmss');
								break;
							default :
								v = '';
								break;
			...
			this.updateOrderHeader = function (orderHeader, success, error) {
				...
				IQ.Debug.info("Current Order Service Data:");
				IQ.Debug.info(self._OrderData);
				...
				_.each(foodItemPostKeys, function (k) {
					...
					switch(k) {
						case 'itemKey':
							...
							break;
						case 'itemType':
							v = KO.TypeDef.OrderFoodItemType.ORDER;
							break;
						case 'foodKey':
						...
						case 'parentFoodFromItemKey':
							...
							break;
						case 'makeStatus':
							v = _.result(_.find(KO.TypeDef.FoodMakeStatus, function (o) {
			...
			this.insertCommonFoodItem = function (food) {
				...
				// 向子窗口推送新加菜品的消息
				KO.SecondScreen.publishPostMsg('OrderDetail', self.getOrderPublishData());
				return item;
			};
			...
			this.submitOrder = function (actionType, payParams, tmpEMP) {
				...
				orderJson = KO.Common.formatPostData(orderJson);
				...
				params = KO.Common.formatPostData(params);

				IQ.Debug.info("Current Post Order Data:");
				IQ.Debug.info(params);
				IQ.Debug.info(JSON.stringify(params));
			...
			this.foodOperation = function (actionType, itemKeys, empInfo) {
				...
				var postData = {
					...
					saasOrderKey : saasOrderKey,
					// foodLst : foodLst
					foodLst : JSON.stringify(KO.Common.formatPostData({foodLst : foodLst})),
					hisFlag : _.result(self._OrderData, 'his', 0)
				};
				...
				IQ.Debug.info("Current Food Operation Post Data:");
				IQ.Debug.info(postData);
			...
			this.suspendOrder = function (successFn, failFn) {
				...
				IQ.Debug.info("Current Order Data:");
				IQ.Debug.info(order);
	...
	app.service('OrderPayService', [
		...
		function ($rootScope, $location, $filter, storage, CommonCallServer, OrderService, PaySubjectService, OrderDiscountRuleService) {
			...
			// 订单支付科目组的数据表
			this.OrderPaySubjectGrpHT = new IQ.IListManager();
			// 结账使用的订单支付科目数据表
			this.OrderPaySubjectHT = new IQ.IListManager();
			// 订单条目金额小计数据表
			this.OrderItemSubTotalHT = new IQ.IListManager();
			...
			var initOrderPaySettings = function () {
				...
				IQ.Debug.info("初始化订单支付相关全局参数:");
				IQ.Debug.info(OrderPaySettingKeys.join('\t'));
				IQ.Debug.info(_.values(_.pick(self, OrderPaySettingKeys)).join('\t'));
				// IQ.Debug.info(self.isVipPrice + "\t" + self.discountRate + "\t" + self.discountRange + "\t" + self.moneyWipeZeroType);
			...
			this.initOrderPay = function (successFn) {
				...
				paySubjectCallServer.success(function () {
					...
					IQ.Debug.info("Pay Subject Group Lst:");
					IQ.Debug.info(paySubjectGrpLst);
					// 支付科目字典数据加载完成后，
				...
				discountRuleCallServer.success(function () {
					...
					IQ.Debug.info("Discount Rule Lst:");
					IQ.Debug.info(discountRuleLst);
				});
			};
			...
			this.updatePaySubjectItem = function (subjectGrpName, params) {
				...
				IQ.Debug.info("OrderPaySubjectLst:");
				IQ.Debug.info(self.OrderPaySubjectHT.getAll());
			};
			...
			this.vipCardDeductMoney = function () {
				...
				consumptionPointAmount = _.filter(payGrps, function (el) {
					var pointKeys = 'cashPay,bankCardPay,kooPay,hangingPay'.split(',');
	...
	app.service('OrderNoteService',[
		...
		function ($rootScope, $location, $filter, $sanitize, $sce, storage, CommonCallServer) {
			IQ.ns("KO");
			var self = this;
			var OrderNoteTypes = KO.TypeDef.OrderNoteTypes;
	...
	app.service('PaySubjectService',[
		...
		function ($rootScope, $location, $filter, $sanitize, $sce, storage, CommonCallServer) {
			IQ.ns("KO");
			var self = this;
			var paySubjectGroups = KO.TypeDef.OrderPaySubjectGroups;
			// var paySubjectGroupNames = '账单赠送,账单会员价优惠,账单折扣,账单减免,现金,银行存款,会员卡,哗啦啦,团购,代金券,挂账,账单元整'.split(',');
			self.paySubjectDict = new IQ.IListManager();
			...
			this.initDiscountRules = function (records) {
				...
				_OrderDiscountRules = _.map(records, function (el) {
					...
					return _.extend(el, {
						...
						value : IQ.id() + ';' + discountRate + ';' + discountRange + ';' + isVipPrice
					});
				});
			};
	...
	app.service('VIPCardService', [
		...
		function ($rootScope, $location, $filter, $sanitize, $sce, storage, CommonCallServer) {
			...
			this.CashVoucherHT = new IQ.IListManager();
			this.ExchangeVoucherHT = new IQ.IListManager();
	...
	app.service('LocalOrderLstService', [
		...
		function ($rootScope, $location, $filter, $sanitize, $sce, storage, CommonCallServer) {
			IQ.ns("KO");
			var self = this;
			var orderHT = new IQ.IListManager(),
				totalSize = 0,
				pageNo = 1,
				pageSize = 15;
	...
	app.service('CloudOrderLstService', [
		...
		function ($rootScope, $location, $filter, $sanitize, $sce, storage, CommonCallServer) {
			...
			var self = this;
			var orderHT = new IQ.IListManager(),
				orderCountInfoHT = new IQ.IListManager(),
				totalSize = 0,
				pageNo = 1,
				pageSize = 6;
	...
	app.service('CloudOrderService', [
		...
		function ($rootScope, $location, $filter, $sanitize, $sce, storage, CommonCallServer) {
			...
			var self = this;
			this._OrderData = null;
			this.OrderFoodHT = new IQ.IListManager();
			this.PaySubjectHT = new IQ.IListManager();
			...
			this.initOrderFoodDB = function (data) {
				...
				_HT.clear();
				_.each(foods, function (food) {
					var itemKey = _.result(food, 'unitCode') + '_' + IQ.id();
					food = _.extend(food, {
						itemKey : itemKey
					});
					// 为字典注册菜品条目数据
			...
			this.initOrderPaySubjectDB = function (data) {
				...
				_.each(paySubjects, function (pay) {
					var itemKey = _.result(pay, 'payName') + '_' + IQ.id();
					_.extend(pay, {
						itemKey : itemKey
					});
			...
			this.refund = function (refundCause) {
				...
				return CommonCallServer.refundCloudOrder({
					orderKey : _.result(self._OrderData, 'orderKey'),
					orderTotal : KO.Common.Math.sub(orderTotalAmount, receivableAmount),
