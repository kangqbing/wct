...
module.exports = function(grunt) {
	...
	// 配置参数
	var banner = "Koo SAAS FE Project";
	var config = {
		src : "src/main/webapp",
		dist : "src/main/app",
		test : "test",
	...
	grunt.initConfig({
		...
		// build性能加速
		concurrent : {
			server : ['less', 'copy:css'],
			test : ['less', 'copy:css'],
			dist : ['less', 'copy:css', 'copy:dist']
		},
		// web 服务器
		connect : {
			options : {
						...
						return [
								...
								connect()
										.use(
												...
												connect
														.static('./src/main/bower_components')),
								connect()
										.use('/test', connect.static('./test')),
								// connect().use('/test',
								// connect.static('./app/test')),
		...
		requirejs : {
			compile : {
				options : {
					...
					modules : [{
						name : "main"
							// include : ["jquery", "underscore", 'IQ',
							// 'commonFn', 'datatype', 'global-const',
							// 'matcher', 'uuid']
						}, {
						...
						include : ['services/appServices',
								'directives/appDirectives',
								'filters/appFilters', 'services/orderServices',
						...
						include : ['directives/appDirectives',
								'services/appServices', 'filters/appFilters'],
						exclude : ["main"]
					}, {
						name : 'dingdan/DingDanViewController',
						include : ['directives/appDirectives',
								'services/appServices',
								'services/orderServices', 'filters/appFilters'],
						exclude : ["main"]
					}, {
						...
						include : ['directives/appDirectives',
								'services/appServices',
								'services/orderServices',
								'services/tableServices', 'filters/appFilters'],
						exclude : ["main"]
					}, {
	...
	grunt
			.registerTask(
					...
					function(target) {
						...
						if (target === 'dist' || target === 'mu'
								|| target === 'dohko') {
							return grunt.task.run(['build:' + target,
									'connect:dist:keepalive']);
						}
						grunt.task.run(['clean:server', 'bower',
								 'concurrent:server',
								'targethtml:dev',
								 'connect:livereload',
								'watch']);
	...
	grunt.registerTask('build', function(target) {
		...
		grunt.task.run(['clean:dist', 'bower', 'concurrent:dist', 'requirejs',
				'clean:disttest', 'targethtml:' + htmlTarget, 'cssmin',
				'connect:dist', 'watch']);
			// if (htmlTarget == 'mu' || htmlTarget == 'dohko' || htmlTarget ==
			// 'dist') {
			// grunt.task.run([
			// 'clean:zip',
	...
	grunt.registerTask('test', function() {
				grunt.task.run(['clean:server', 'less', 'bower',
						'concurrent:test', 'connect:test', 'karma']);
			});

	// 生成打包文件所需版本号
	grunt.registerTask('genSVNRevision',
			"Generate SVN Revision to zip file name", function(target) {
				var done = this.async(), exec = require('child_process').exec, child;
				child = exec("svn info|grep Revision |sed 's/.* //'", {
							...
							var date = new Date(), y = date.getFullYear(), m = date
									.getMonth()
									+ 1, d = date.getDate(), revision = parseInt(stdout);
							m = m > 9 ? m : ('0' + m);
							d = d > 9 ? d : ('0' + d);
							grunt.log.ok(stdout);
							grunt.log.ok(revision);
							config.zipFileName = target + '.SAAS-FE.' + y + m
									+ d + '_' + revision + '.tar.gz';
							grunt.log.ok("The zip file name is "
									+ config.zipFileName);
							done(true);
						});
			});
	...
	grunt.registerTask('genRevision', "Generate revision file", function() {
		var done = this.async(), fileName = config.zipFileName, revisionFile = './revision';
		...
		fs.exists(revisionFile, function(exists) {
			...
			fs.open(revisionFile, 'w+', function(err, fd) {
				...
				var buffer = new Buffer(revisionValue), bufferLength = buffer.length, filePosition = null;
				fs.write(fd, buffer, 0, bufferLength, filePosition, function(
								err, written) {
							if (err) {
								throw err;
							}
	...
	grunt.registerTask('zip', "Zip builded files to dist.tar.gz", function() {
				var done = this.async(), exec = require('child_process').exec, child;
				var fileName = config.zipFileName, cmd = 'tar -zcvf '
						+ fileName + ' ./dist ./revision';
